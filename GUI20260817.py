...
class GaussianToolGUI(TkinterDnD.Tk):
    ...
    def create_console_widgets(self):
        ...
        # 终端区域
        terminal_frame = ttk.LabelFrame(self.param_frame, text="终端(伪)   ps:时刻牢记使用英文输入法在终端键入文字",
                                        padding=5)
    ...
    def create_modify_gjf_widgets(self):
        ...
        ttk.Button(self.param_frame, text="远程",
                   command=lambda: self.select_remote_folder(self.input_folder_var)).grid(
        ...
        ttk.Button(self.param_frame, text="浏览", command=lambda: self.select_folder(self.output_folder_var)).grid(
            row=row, column=2, padx=5, pady=2)
        ttk.Button(self.param_frame, text="远程",
                   command=lambda: self.select_remote_folder(self.output_folder_var)).grid(
            row=row, column=3, padx=5, pady=2)
        self.current_widgets.extend([entry, self.param_frame.grid_slaves(row=row, column=0)[0],
                                     self.param_frame.grid_slaves(row=row, column=2)[0]])
        row += 1
    ...
    def create_reorg_widgets(self):
        ...
        rb_local = ttk.Radiobutton(source_frame, text="本地文件 (上传)", variable=self.reorg_source_var,
                                   value="local", command=self._toggle_reorg_input)
        rb_local.pack(side=tk.LEFT, padx=10)
        rb_remote = ttk.Radiobutton(source_frame, text="远程已有文件", variable=self.reorg_source_var,
        ...
        # 远程工作目录
        ttk.Label(self.param_frame, text="远程工作目录 (必填):").grid(row=row, column=0, sticky=tk.W, padx=5, pady=2)
        self.reorg_workdir_var = tk.StringVar(value="")
        workdir_entry = ttk.Entry(self.param_frame, textvariable=self.reorg_workdir_var, width=40)
        workdir_entry.grid(row=row, column=1, padx=5, pady=2, sticky="ew")
        ...
        self.reorg_params = {
            'g': tk.StringVar(value="b3lyp"),
            'o': tk.StringVar(value="b3lyp/G"),
            'gb': tk.StringVar(value="6-31G(d,p)"),
            'ob': tk.StringVar(value=""),
            'root': tk.StringVar(value="1"),
            'sm': tk.StringVar(value="1"),
        ...
        ttk.Button(btn_frame, text="提交计算任务", command=self.submit_reorg_job).pack(side=tk.LEFT, padx=10)

        self.param_frame.grid_rowconfigure(row + 1, weight=1)
        self.param_frame.grid_columnconfigure(1, weight=1)

    def _toggle_reorg_input(self):
        """根据来源切换显示本地或远程输入框"""
        if self.reorg_source_var.get() == "local":
            self.reorg_local_frame.pack(fill=tk.X, expand=True, pady=2)
            self.reorg_remote_frame.pack_forget()
        else:
            self.reorg_remote_frame.pack(fill=tk.X, expand=True, pady=2)
            self.reorg_local_frame.pack_forget()
    ...
    def _browse_workdir(self):
        ...
        if not self.sftp:
            self.log("请先连接远程服务器")
            return
        dir_path = self.choose_remote_directory()
        if dir_path:
    ...
    def submit_reorg_job(self):
        ...
        # 工作目录
        workdir = self.reorg_workdir_var.get().strip()
        if not workdir:
            self.log("请指定远程工作目录（必填）")
            return
        try:
            self.sftp.stat(workdir)
        except FileNotFoundError:
            self.log(f"远程工作目录不存在: {workdir}")
            return
        except Exception as e:
            self.log(f"访问远程目录失败: {e}")
            return
        ...
        ob_val = self.reorg_params['ob'].get().strip() or gb_val
        root_val = self.reorg_params['root'].get().strip() or "1"
            ...
            try:
                ...
                def read_stream(stream, tag):
                    for line in iter(stream.readline, ''):
                        self.after(0, lambda l=line: self.log(f"[{tag}] {l.rstrip()}"))
                ...
                self.after(0, lambda: self.log(f"任务结束，退出码: {exit_status}"))
            except Exception as e:
                self.after(0, lambda: self.log(f"执行远程命令失败: {e}"))
...
class FTPWindow(tk.Toplevel):
    ...
    def _update_progress(self, transferred, total):
        ...
        self.progress_label.config(
            text=f"{transferred / (1024 * 1024):.1f} MB / {total / (1024 * 1024):.1f} MB ({percent:.1f}%)")
            ...
            with os.scandir(path) as entries:
                ...
                for entry in items:
                    ...
                    if entry.is_dir():
                        ...
                        type_str = "文件"
                        size_str = self.master._format_size(entry.stat().st_size) if hasattr(self.master,
                                                                                             '_format_size') else ""
                        tags = ("file",)
